...
{rows(columns, '02_コラム・モデル', False)}
 ...
 <div class="note">
  ...
  <b>更新のしかた</b><br>
  新しい完全版を追加したらバックアップ用スクリプトを再実行してください。フォルダごと作り直され、この索引も更新されます。<br><br>
  <b>運用設定</b><br>
  ドメイン・リポジトリ・note・X・計測まわりの設定は <code>ブログ運用設定.md</code> にまとめてあります。
  パスワードとAPIトークンは意図的に記載していません（前者はパスワードマネージャ、後者は <code>.cf_token</code>）。<br><br>
  <b>note原稿</b><br>
  <code>04_note原稿/</code> にnote用の原稿を置いています。<b>有料記事の原稿を含むため、公開リポジトリには入れません</b>
  （このバックアップはローカル専用）。
 </div>
</div></body></html>
"""
...
├── README.md           ← このファイル
├── ブログ運用設定.md   ← ドメイン/リポジトリ/note/X/計測の設定メモ（認証情報は書かない）
